...
const PlayersStats: React.FC<PlayersStatsProps> = ({ players, buildings, attackPlayers = [] }) => {
  const stats = React.useMemo(() => {
    ...
    const assignedAttackPlayers = attackPlayers.filter(ap =>
      buildings.some(building =>
        building.capitan?.id === ap.id ||
        building.players.some(pl => pl.player.id === ap.id)
      )
    );
    ...
    return {
      ...
      assignedAttack: assignedAttackPlayers.length
    };
  ...
  return (
    <Card className="mb-3">
      ...
      <Card.Body>
        <Row className="flex-nowrap justify-content-evenly">
          <Col xs={6} md="auto">
          ...
          <Col xs={6} md="auto">
            <div className="text-center">
              <h4>{stats.assignedAttack}</h4>
              <small className="text-muted">Attack Players Assigned to Defense</small>
              <div className="mt-2">
                <OverlayTrigger
                  placement="top"
                  overlay={<Tooltip id="assigned-attack-players-tooltip">Attack players assigned to defense</Tooltip>}
                >
                  <Badge bg="danger">{stats.assignedAttack}</Badge>
                </OverlayTrigger>
              </div>
            </div>
          </Col>
          ...
          <Col xs={6} md="auto">
            <div className="text-center">
              ...
              <small className="text-muted">Buildings Total</small>
              <div className="mt-2">
                <OverlayTrigger
                  placement="top"
